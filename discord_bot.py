...
class GameCommands(commands.Cog):
    ...
    async def join_game(self, ctx):
        ...
        if self.bot.game.add_player(ctx.author.id, ctx.author.display_name):
            ...
            embed.add_field(name="Players", value=len(self.bot.game.players), inline=True)
            embed.add_field(name="Status", value=self.bot.game.state.value.title(), inline=True)
    ...
    async def start_game(self, ctx):
        ...
        if self.bot.game.start_game():
            ...
            embed.add_field(name="Players", value=len(self.bot.game.players), inline=True)
            embed.add_field(name="Current 'It'", value=self.bot.game.current_it.discord_name, inline=True)
            embed.add_field(name="Duration", value=f"{self.bot.game.round_duration // 60} minutes", inline=True)
            await ctx.send(embed=embed)
        else:
    ...
    async def game_status(self, ctx):
        ...
        embed.add_field(name="Current 'It'", value=status["current_it"] or "None", inline=True)
        embed.add_field(name="Active Challenges", value=status["active_challenges"], inline=True)
        embed.add_field(name="Game Time", value=f"{status['game_time']:.0f}s", inline=True)
        embed.add_field(name="Round Duration", value=f"{status['round_duration']}s", inline=True)
        await ctx.send(embed=embed)
    ...
    async def tag_player(self, ctx, target):
        ...
        if result["success"]:
            ...
            embed.add_field(name="Time Limit", value=f"{self.bot.config['game_settings']['dodge_timeout']}s", inline=True)
